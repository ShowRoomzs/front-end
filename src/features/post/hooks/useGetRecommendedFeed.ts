...
export function useGetRecommendedFeed(enabled = true) {
  return useInfiniteList<FeedItem>({
    queryKey: [POST_QUERY_KEY.RECOMMENDED_FEED],
    queryFn: page => postService.getRecommendedFeed({ page, size: PAGE_SIZE }),
    enabled,
  });
}
